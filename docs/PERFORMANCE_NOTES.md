...
## Фотоотчеты — производительность и офлайн

- Ленивая загрузка миниатюр с прогрессивным блюром (LazyImage)
- Миниатюры кешируются в IndexedDB (WebP с fallback на JPEG)
- Предзагрузка соседних фото в просмотрщике (next/prev)
- Готов компонент `VirtualizedPhotoGrid` для больших сеток
- Service Worker (`public/sw.js`) кэширует внешние изображения (Supabase Storage) и статические ассеты
